...
export const Navigation: React.FC = () => {
  ...
  const navItems = [
    ...
    { path: '/reports-dashboard', label: 'Relatórios', icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/><line x1="12" x2="12" y1="13" y2="17"/><line x1="15" x2="9" y1="15" y2="15"/></svg> },
  ...
  return (
    <>
      ...
      <div className={`fixed bottom-24 right-6 z-40 flex flex-col items-end
         space-y-4 transition-all duration-300 ${scrollDirection === 'down' || isFabMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}>
        {isFabMenuOpen && (
          <div className="flex flex-col items-end space-y-4">
            ...
            <div className="flex items-center space-x-3">
              ...
              <Link to="/add-patient" onClick={() => setIsFabMenuOpen(false)} className="w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-lg text-[#00C4B4]">{fabIcons.patient}</Link> {/* ATUALIZADO: Link para AddPatientPage */}
            </div>
            <div className="flex items-center space-x-3">
              <span className="bg-white p-2 rounded-lg shadow-md text-gray-700 font-semibold">PDFs Gerados</span>
              <Link to="/generated-pdfs" onClick={() => setIsFabMenuOpen(false)} className="w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-lg text-[#00C4B4]">{fabIcons.report}</Link>
            </div>
            <div className="flex items-center space-x-3">
        ...
        <button
          ...
          className={`w-16 h-16 bg-[#00C4B4] rounded-full flex items-center
            justify-center text-white shadow-2xl transform transition-transform duration-300
      ...
      <nav className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-t-lg z-20
        transition-transform duration-300 ${scrollDirection === 'down' && !isFabMenuOpen ? 'translate-y-full' : 'translate-y-0'}`}>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`flex flex-col items-center justify-center text-center
                  w-20 h-16 rounded-lg transition-colors ${
                    location.pathname.startsWith(item.path) && item.path !== '#'
                    ? 'text-[#00C4B4] bg-[#E8F5F4]'
                    : 'text-gray-500 hover:text-[#00C4B4] hover:bg-gray-50'
                  }`}
              >
                <NavIcon isActive={location.pathname.startsWith(item.path) && item.path !== '#'}>
                  {item.icon}
                </NavIcon>
