...
// Ícones para o menu flutuante e de navegação
const fabIcons = {
    scan: <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><path d="M3 7V5a2 2 0 0 1 2-2h2" /><path d="M17 3h2a2 2 0 0 1 2 2v2" /><path d="M21 17v2a2 2 0 0 1-2 2h-2" /><path d="M7 21H5a2 2 0 0 1-2-2v-2" /><path d="M7 12h10" /></svg>,
    ...
    // *** NOVO ÍCONE PARA 'CASOS' ***
    cases: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><line x1="3" y1="9" x2="21" y2="9" /><line x1="9" y1="21" x2="9" y2="9" /></svg>,
};
...
export const Navigation: React.FC = () => {
    ...
    // *** LISTA DE ITENS DA NAVEGAÇÃO ATUALIZADA ***
    const navItems = [
        ...
        // *** ITEM 'ANÁLISE' SUBSTITUÍDO POR 'CASOS' ***
        { path: '/cases', label: 'Casos', icon: fabIcons.cases },
    ...
    return (
        <>
            ...
            <div className={`fixed bottom-24 right-6 z-40 flex flex-col items-end space-y-4 transition-all duration-300 ${scrollDirection === 'down' ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10 pointer-events-none'}`}>
                {isFabMenuOpen && (
                    <div className="flex flex-col items-end space-y-4">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <Link to="/patients/new" onClick={() => setIsFabMenuOpen(false)} className="w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-lg text-[#00C4B4]">{fabIcons.patient}</Link>
                        </div>
                    </div>
                )}
                <button
                ...
                >
                    {isFabMenuOpen ? fabIcons.close : fabIcons.scan}
                </button>
            </div>
                ...
                <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
                    <div className="flex justify-around py-1">
                        {navItems.map((item) => (
                            ...
                            >
                                <NavIcon isActive={location.pathname.startsWith(item.path)}>
                                    {item.icon}
                                </NavIcon>
